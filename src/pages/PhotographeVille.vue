  ...
  <div class="bg-[#FFFBF8] text-[#2E2B29] min-h-screen" v-if="city">
    ...
    <nav class="max-w-6xl mx-auto px-4 pt-4 pb-2 text-sm text-[#2E2B29]/60">
      <ol class="flex flex-wrap items-center gap-1">
        ...
        <li>
          <nuxt-link to="/photographe-moselle" class="hover:text-[#C2A191] transition-colors">Photographe Moselle</nuxt-link>
        </li>
        <li class="text-[#C2A191]">/</li>
...
const H1_VARIANTS = [
  ...
  (c) => `Photographe VDL à ${c.name} en ${c.department}`,
  (c) => `Photographe photo à ${c.name} - Valeria Daniele Leonardi`,
  (c) => `Photographe de vos souvenirs à ${c.name}`,
  (c) => `Photographe mariage et famille à ${c.name}`,
  (c) => `Valeria Daniele Leonardi, photographe à ${c.name}`,
];

const META_TITLE_VARIANTS = [
  (c) => `Photographe Mariage Famille ${c.name}${c.postalCode ? ' (' + c.postalCode + ')' : ''}`,
  (c) => `Photographe ${c.name}${c.postalCode ? ' (' + c.postalCode + ')' : ''} - Valeria Daniele Leonardi`,
  (c) => `Photographe Grossesse Naissance ${c.name}`,
];
